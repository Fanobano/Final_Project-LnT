    ...
    <div class="cards-container">
        @foreach ($books as $b)
            <div class="card">
                <div class="card-body">
                    @if($b->image)
                        <img src="{{ asset('/storage/book/'.$b->image)}}" alt="" class="card-img-top">
                    @else
                    <p>No image...</p>
                    @endif
                    <h5 class="card-title">{{ $b->name }}</h5>
                    <p class="card-text" style="margin-bottom: 20px;">Price: Rp.{{ $b->price}}</p>
                    <p class="card-text" style="margin-bottom: 20px;">Category: {{ $b->category}}</p>
                    <p class="card-text" style="margin-bottom: 20px;">Stock: {{ $b->stock }}</p>
                    <div class="d-flex flex-column">
                        ...
                        <form method="POST" action="{{ route('delete_image', ['id'=>$b->id]) }}">
                            @csrf
                            @method('DELETE')
